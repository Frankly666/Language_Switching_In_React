...
function checked(curProject, map, row) {
  const filePaths = glob.sync(
    config.include.map((value) => curProject + value),
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (hanRegex.test(str)) {
        ...
        while (_line.match(hanRegex)) {
          ...
          const id = `${filePath.replace(/[\/\\\.]/g, "_")}-${i}-${
            ChWordInfo.index
          }`.slice(4);
          row.push({
            [id]: ChWordInfo[0],
          ...
          _line = _line.replace(
            hanRegex,
            new Array(ChWordInfo[0].length).fill("-").join("")
          );
          map.push({
            path: filePath,
            id,
...
for (const project of projects) {
  ...
  fs.writeFileSync(
    path.join(distMapPath, `${project}.map.json`),
    JSON.stringify(map),
    {
      encoding: "utf-8",
      flag: "w",
  ...
  fs.writeFileSync(
    path.join(distRowPath, `${project}.dic.json`),
    JSON.stringify(row),
    {
      encoding: "utf-8",
      flag: "w",
